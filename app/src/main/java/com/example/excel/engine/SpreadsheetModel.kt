...
class SpreadsheetWorkbook(
    ...
    init {
        require(this.sheets.isNotEmpty()) { "A workbook must contain at least one sheet" }
    }
...
class SpreadsheetSheet(var name: String) {
    ...
    fun setValue(address: CellAddress, value: CellValue) {
        if (value is CellValue.Empty) cells.remove(address) else cell(address).value = value
    }

    fun clear(range: CellRange) {
        cells.keys.removeIf(range::contains)
    }

    fun cellsIn(range: CellRange): Sequence<Pair<CellAddress, SpreadsheetCell>> =
        cells.asSequence().filter { range.contains(it.key) }

    fun insertRow(row: Int) {
        require(row >= 0)
        shiftCells { a -> if (a.row >= row) a.copy(row = a.row + 1) else a }
    }
    ...
    fun deleteRow(row: Int) {
        ...
        cells.keys.toList().forEach { a ->
            when {
                ...
                a.row > row -> {
                    val value = cells.remove(a) ?: return@forEach
                    cells[a.copy(row = a.row - 1)] = value
                }
            }
        }
    }

    fun insertColumn(column: Int) {
        require(column >= 0)
        shiftCells { a -> if (a.column >= column) a.copy(column = a.column + 1) else a }
    }
    ...
    fun deleteColumn(column: Int) {
        ...
        cells.keys.toList().forEach { a ->
            when {
                ...
                a.column > column -> {
                    val value = cells.remove(a) ?: return@forEach
                    cells[a.copy(column = a.column - 1)] = value
                }
            }
        }
    }
    ...
    fun merge(range: CellRange) { require(range.isSingleCell.not()); mergedRanges += range }
...
data class CellRange(
    ...
    fun addresses(): Sequence<CellAddress> = sequence {
        for (r in top..bottom) for (c in left..right) yield(CellAddress(r, c))
    }

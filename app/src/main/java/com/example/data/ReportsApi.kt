            ...
            if (data != null) for (i in 0 until data.length()) {
              data.optJSONObject(i)?.let { item ->
                add(ExcelReport(
                  ...
                  groupName = item.optString("group_name", "Report"),
    ...
    Thread {
      try {
        ...
        if (mimeType !in supported) {
          onError("फक्त Excel, CSV किंवा PDF Report upload करता येईल.")
          return@Thread
        }
        val bytes = resolver.openInputStream(uri)?.use { it.readBytes() }
        if (bytes == null || bytes.isEmpty()) {
          onError("Report file वाचता आला नाही.")
          return@Thread
        }
        if (bytes.size.toLong() > MAX_UPLOAD_BYTES) {
          onError("Report file 50 MB पेक्षा मोठी असू शकत नाही.")
          return@Thread
        }
        ...
        val request = Request.Builder()
          .url("$BASE_URL/api/excel/reports/upload")
        ...
        client.newCall(request).execute().use { response ->
          val json = runCatching { JSONObject(response.body?.string().orEmpty()) }.getOrElse { JSONObject() }
          if (!response.isSuccessful || !json.optBoolean("success", false)) {
            onError(json.optString("error").ifBlank { "Report upload अयशस्वी (HTTP ${response.code})" })
            return@use
          }
          val item = json.optJSONObject("data") ?: JSONObject()
          onSuccess(ExcelReport(
            id = item.optString("id"),
            groupId = item.optString("group_id", groupId),
            groupName = item.optString("group_name", "Report"),
            senderName = item.optString("sender_name", ""),
            fileName = item.optString("file_name", name),
      ...
      try {
        ...
        client.newCall(request).execute().use { response ->
          ...
          try {
            ...
            onError("ही Report उघडण्यासाठी योग्य app उपलब्ध नाही.")
          }
        }
      } catch (e: Exception) {

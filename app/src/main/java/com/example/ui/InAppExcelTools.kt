...
internal object InAppExcelFormula{
  ...
  private fun range(a:String,b:String):List<String>{val x=Regex("([A-Z]+)([0-9]+)",RegexOption.IGNORE_CASE).matchEntire(a.trim())?:return emptyList();val y=Regex("([A-Z]+)([0-9]+)",RegexOption.IGNORE_CASE).matchEntire(b.trim())?:return emptyList();fun c(s:String)=s.uppercase().fold(0){n,ch->n*26+ch.code-64};fun n(v0:Int):String{var v=v0;val o=StringBuilder();while(v>0){val r=(v-1)%26;o.append(('A'.code+r).toChar());v=(v-1)/26};return o.reverse().toString()};val c1=c(x.groupValues[1]);val c2=c(y.groupValues[1]);val r1=x.groupValues[2].toInt();val r2=y.groupValues[2].toInt();return buildList{for(r in minOf(r1,r2)..maxOf(r1,r2))for(col in minOf(c1,c2)..maxOf(c1,c2))add("${n(col)}$r")}}
}

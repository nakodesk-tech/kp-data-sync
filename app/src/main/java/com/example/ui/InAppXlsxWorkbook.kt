...
internal data class InAppSheet(val name: String, val entryName: String, val cells: MutableList<MutableList<String>>)
...
internal class InAppXlsxWorkbook private constructor(
  ...
  private fun updateSheet(bytes: ByteArray, changed: Map<String, String>): ByteArray {
    ...
    val sheetData = doc.documentElement.getElementsByTagName("sheetData").item(0) as? Element ?: return bytes
    changed.forEach { (ref, value) ->
      ...
      if (row == null) { row = doc.createElement("row"); row.setAttribute("r", rowNumber.toString()); sheetData.appendChild(row) }
      var cell = findCell(row, ref)
      if (cell == null) { cell = doc.createElement("c"); cell.setAttribute("r", ref); row.appendChild(cell) }
      ...
      textNode.appendChild(doc.createTextNode(value)); isNode.appendChild(textNode); cell.appendChild(isNode)
    }
  ...
  private fun cellRef(row: Int, column: Int): String {
    var n = column; val sb = StringBuilder()
  ...
  companion object {
    fun load(file: File): Result<InAppXlsxWorkbook> = runCatching {
      ...
      if (rels != null) {
        val doc = parseXmlStatic(rels); val nodes = doc.getElementsByTagName("Relationship")
        for (i in 0 until nodes.length) {
          val e = nodes.item(i) as Element; val id = e.getAttribute("Id"); val target = e.getAttribute("Target")
          if (id.isNotBlank() && target.isNotBlank()) relMap[id] = if (target.startsWith("/")) target.removePrefix("/") else "xl/$target"
        }
      }
      val wbDoc = parseXmlStatic(workbook); val sheetNodes = wbDoc.getElementsByTagName("sheet"); val sheets = mutableListOf<InAppSheet>()
      for (i in 0 until sheetNodes.length) {
        ...
        entries[entryName]?.let { sheets += parseSheet(name, entryName, it, shared) }
      }
    ...
    private fun parseSharedStrings(bytes: ByteArray?): List<String> {
      ...
      val doc = parseXmlStatic(bytes); val nodes = doc.getElementsByTagName("si")
      return buildList { for (i in 0 until nodes.length) { val ts = (nodes.item(i) as Element).getElementsByTagName("t"); add(buildString { for (j in 0 until ts.length) append(ts.item(j).textContent) }) } }
    }

    private fun parseSheet(name: String, entryName: String, bytes: ByteArray, shared: List<String>): InAppSheet {
      val doc = parseXmlStatic(bytes); val rows = doc.getElementsByTagName("row"); val matrix = mutableListOf<MutableList<String>>(); var maxColumn = 0
      for (i in 0 until rows.length) {
        val rowElement = rows.item(i) as Element; val rowNumber = rowElement.getAttribute("r").toIntOrNull() ?: (i + 1)
        while (matrix.size < rowNumber) matrix.add(mutableListOf())
        val row = matrix[rowNumber - 1]; val cells = rowElement.getElementsByTagName("c")
        for (j in 0 until cells.length) {
          val cell = cells.item(j) as Element; val ref = cell.getAttribute("r")

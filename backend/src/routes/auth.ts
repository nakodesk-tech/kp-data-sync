...
authRouter.post('/setup-admin', async (c) => {
  ...
  try {
    ...
    if (!serverSetupSecret) {
      console.error('SETUP_ADMIN_ERROR [secret configuration]: SETUP_SECRET is not configured.');
      return c.json({
        success: false,
    ...
    if (!c.env.DB) {
      console.error('SETUP_ADMIN_ERROR [database binding]: D1 binding DB is not configured.');
      return c.json({
        success: false,
        error: 'Server configuration error: D1 database binding DB is not configured.'
      }, 500);
    }
    ...
    try {
      ...
      console.error('SETUP_ADMIN_ERROR [users table preflight]:', error);
      return c.json({
        success: false,
        error: `Database error during users table check: ${error?.message || 'unknown database error'}`
      }, 500);
    }
    ...
    const body = await c.req.json().catch(() => ({}));
    if (!providedSecret && typeof body.setup_secret === 'string') {
    ...
    let adminCountRow;
    try {
      adminCountRow = await c.env.DB
        .prepare("SELECT COUNT(*) as count FROM users WHERE role = 'Admin'")
        .first<{ count: number }>();
    } catch (error: any) {
      console.error('SETUP_ADMIN_ERROR [admin count query]:', error);
      return c.json({
        success: false,
        error: `Database error during Admin check: ${error?.message || 'unknown database error'}`
      }, 500);
    }
    ...
    try {
      ...
      console.error('SETUP_ADMIN_ERROR [email duplicate check]:', error);
      return c.json({
        success: false,
        error: `Database error during email check: ${error?.message || 'unknown database error'}`
      }, 500);
    }
    ...
    try {
      ...
      console.error('SETUP_ADMIN_ERROR [password hashing]:', error);
      return c.json({
        success: false,
        error: `Password hashing failed: ${error?.message || 'unknown error'}`
      }, 500);
    }
    ...
    try {
      ...
      return c.json({
        success: false,
        error: `Database error during Admin creation: ${error?.message || 'unknown database error'}`
      }, 500);
    }
    ...
    return c.json({
      success: false,
      error: `Admin bootstrap failed at ${stage}: ${error?.message || 'Internal server error'}`
    }, 500);
  }
});

...
userRouter.post('/register', authMiddleware(['Admin', 'Cluster_Head', 'School_HM']), async (c) => {
  ...
  let body: {
    name?: string; email?: string; mobile_number?: string; password?: string; role?: UserRole;
    cluster_name?: string; cluster_code?: string; school_name?: string; school_code?: string; address?: string;
  };
  ...
  let resolvedSchoolCode = schoolCode;

  try {
    if (schoolCode) {
      const school = await c.env.DB.prepare(`
        SELECT school_name, udise_code, cluster_name, cluster_code, is_active
        FROM schools WHERE udise_code = ? OR id = ? LIMIT 1
      `).bind(schoolCode, schoolCode).first<{
        school_name: string; udise_code: string | null; cluster_name: string | null; cluster_code: string | null; is_active: number | null;
      }>();
      ...
      resolvedSchoolCode = school.udise_code || schoolCode;
      resolvedClusterName = school.cluster_name || resolvedClusterName;
      resolvedClusterCode = school.cluster_code || resolvedClusterCode;
    ...
    try {
      await c.env.DB.prepare(`
        INSERT INTO users (
          ...
          cluster_name, cluster_code, school_name, school_code, address, status
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active')
      `).bind(
        id, name, email, mobile, passwordHash, requestedRole,
        resolvedClusterName, resolvedClusterCode, resolvedSchoolName, resolvedSchoolCode, address
      ).run();
    } catch (error: any) {
      console.error('User registration INSERT failed:', error);
      return c.json({ success: false, error: `Database error while creating user: ${error?.message || 'unknown database error'}` }, 500);
    }

    return c.json({ success: true, data: {
      id, name, email, role: requestedRole, mobile_number: mobile,
      cluster_name: resolvedClusterName, cluster_code: resolvedClusterCode,
      school_name: resolvedSchoolName, school_code: resolvedSchoolCode,
      address, status: 'Active'
    } }, 201);
  } catch (error: any) {
    console.error('User registration failed:', error);
    return c.json({ success: false, error: `Database error during user registration: ${error?.message || 'unknown database error'}` }, 500);
  }
});

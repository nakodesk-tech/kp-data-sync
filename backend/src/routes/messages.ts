...
async function groupAccess(db: D1Database, actor: any, groupId: string) {
  const group = await db.prepare(`
    SELECT id, group_name, group_type, scope_type, cluster_code, school_code, is_active
...
messageRouter.post('/:id/attachment', async (c) => {
  ...
  return c.json({
    ...
    data: {
      ...
      file_name: safeName,
      mime_type: mimeType,
      file_size: object?.size || (declaredSize || null),
...
messageRouter.get('/:id/realtime', async (c) => {
  ...
  headers.set('X-Chat-User-Id', actor.id);
  headers.set('X-Chat-User-Name', actor.name);
  headers.set('X-Chat-User-Role', actor.role);
  headers.set('X-Chat-Group-Id', groupId);
